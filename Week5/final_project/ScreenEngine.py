...
class GameSurface(ScreenHandle):
    ...
    def draw_map(self):
        ...
        # _FIXME || calculate (min_x,min_y) - left top corner
...
class ProgressBar(ScreenHandle):
    ...
    def connect_engine(self, engine):
        # _FIXME save engine and send it to next in chain
        self.engine = engine
        if self.successor is not None:
            return self.successor.connect_engine(engine)
...
class InfoWindow(ScreenHandle):
    ...
    def connect_engine(self, engine):
        # _FIXME set this class as Observer to engine and send it to next in
        # chain
        self.engine = engine
...
class MiniMap(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        self.fill((0, 0, 0, alpha))
        if self.game_engine.show_minimap:
            for i in range(len(self.game_engine.map[0]) - 0):
                for j in range(len(self.game_engine.map) - 0):
                    self.blit(self.game_engine.map[0 + j][0 + i][
                              0], (i * self.game_engine.sprite_size // 14, j * self.game_engine.sprite_size // 5.5))
